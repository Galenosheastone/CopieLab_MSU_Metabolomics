...
# Load the dataset
file_path = 'Early_vs_Severe_helo_only_edited_FOR_TESTING.csv'
...
# Replace NaN with 1/5th of the minimum positive value of the corresponding variable
data.iloc[:, 2:] = data.iloc[:, 2:].apply(lambda x: x.fillna(min_positive_values[x.name]))
...
processed_file_path = 'processed_Early_vs_Severe_helo_only.csv'
...
plt.savefig(combined_density_plot_file)
plt.close()
